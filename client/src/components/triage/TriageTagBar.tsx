    ...
    <div className="triage-section">
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <div className="flex flex-wrap gap-1 flex-1 min-w-0">
          {tags.map((tag) => (
            <button
              ...
              disabled={isPending}
              className="text-[10.5px] px-1.5 py-[1px] rounded-full font-medium inline-flex items-center gap-0.5 hover:opacity-70 transition-opacity disabled:cursor-wait"
        ...
        <button
          ...
          title={expanded ? 'Close tag picker' : 'Manage tags'}
        >
          {expanded ? <X size={13} style={{ color: 'var(--text-muted)' }} /> : <Plus size={13} style={{ color: 'var(--text-muted)' }} />}
        </button>
      </div>
      ...
      {expanded && (
        <div className="rounded-lg p-2 mt-1 flex flex-col gap-1.5" style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border)' }}>
          {allTags.length > 0 && (
              ...
              {allTags.map((tag) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => onToggle(tag.id)}
                    disabled={isPending}
                    className="text-[10.5px] px-1.5 py-[1px] rounded-full font-medium inline-flex items-center gap-0.5 transition-opacity disabled:cursor-wait"
                    style={{
